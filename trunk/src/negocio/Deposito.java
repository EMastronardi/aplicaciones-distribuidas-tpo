...
public class Deposito {
	...
	public void sumarInventario(float cant, Lote l){
		...
		auxMovimientos.add(new Movimiento(new Date(),this,l,cant,"Incremento de Stock","prueba"));
		MovimientoDAO.getInstancia().grabarMovimientos(auxMovimientos);
	}
	
	public void restarInventario(Producto p, float cant){
		ArrayList<Movimiento> auxMovimientos = new ArrayList<Movimiento>();
		List<Object[]> cantidadLote = getStockProducto(p); 
		while(cant>0 && cantidadLote!=null){
			...
			if(cant<=cantidadLoteMasViejo){
				auxMovimientos.add(new Movimiento(new Date(),this,(Lote) masViejo[0],cant*-1,"Resta de Stock","prueba"));
				cant = 0;
			}
			else{
				if(cant>cantidadLoteMasViejo){
					...
					l.setEstado(false);
					auxMovimientos.add(new Movimiento(new Date(),this,l,cantidadLoteMasViejo*-1,"Resta de Stock","prueba"));
	...
	private List<Object[]> getStockProducto(Producto p) {
		List<Object[]> loteCantidad = new ArrayList<Object[]>();
		Boolean find = false;
